...
class GoogleApiService:
    ...
    def get_distance_matrix(self, origins, destinations, travel_mode, departure_time=None):
        """
        [升級版] 智慧判斷是否要傳送 departure_time 參數，以避免 'in the past' 錯誤。
        """
        api_params = {
            "origins": origins,
        ...
        # 只有當 departure_time 存在且在未來時，才加入該參數
        if departure_time and departure_time > datetime.now():
            api_params["departure_time"] = departure_time
        
        # 如果 departure_time 是過去式或不存在，我們就不傳入該參數，
        # Google API 會自動回傳基於一般路況的平均時間。
        
        try:
            # 使用 **api_params 將字典解包成關鍵字參數傳入
            return self.gmaps.distance_matrix(**api_params)
        except Exception as e:
            print(f"計算交通時間矩陣時發生錯誤: {e}")
            return None
